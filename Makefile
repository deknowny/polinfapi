...
# Reformat code style
format:
	$(poetry_exec) run black polinfapi && \
	git add -u && \
	$(poetry_exec) run isort polinfapi && \
	git add -u && \
	$(poetry_exec) run autoflake \
		--ignore-init-module-imports \
		--remove-unused-variables \
		--recursive \
		--in-place polinfapi tests && \
	git add -u

# Run tests locally
test:
	$(poetry_exec) run pytest tests --cov=polinfapi --cov-report=html

# Tests command for CI with .coveragerc report
test-ci:
	$(poetry_exec) run coverage run --source=polinfapi -m pytest tests
